...
import { resendConfirmationEmail } from "@/utils/actions/auth";
...
export default function EmailConfirmationPage({ 
  ...
  useEffect(() => {
    ...
    } else if (errorParam?.startsWith('rate_limited:')) {
      const waitTime = parseInt(errorParam.split(':')[1]) || 60;
      setRateLimitCountdown(waitTime);
      setSubmitError(`Too many requests. Please wait ${waitTime} seconds before trying again.`);
  ...
  async function handleResendEmail(formData: FormData) {
    ...
    // Server action will redirect on success or error, so we don't need try/catch
          ...
          <form action={handleResendEmail} className="space-y-4 mt-4">
            ...
            >
              {isSubmitting ? (
                ...
                `Wait ${rateLimitCountdown}s before retrying`
              ) : (
                "Resend confirmation email"
              )}
            </Button>
